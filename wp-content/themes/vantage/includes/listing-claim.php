...
class VA_Claim_Listing extends VA_Checkout_Step {
	...
	function add_claim_to_order( $order ) {
		$order->add_item( VA_LISTING_CLAIM_ITEM, 0, va_get_claimed_listing()->post->ID );
	}
...
class VA_Claim_Listing_Categories extends VA_Checkout_Step {
	...
	function add_category_selections( $order, $post_id ) {
		...
		add_post_meta( $order->get_id(), 'claim_custom_forms', $custom_forms, true );
...
class VA_Claim_Listing_Select_Plan extends VA_Checkout_Step{
