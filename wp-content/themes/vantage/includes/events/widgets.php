...
class VA_Widget_Event_Attendees extends WP_Widget {
	...
	static function handle_ajax() {
		...
		if ( is_user_logged_in() ) {
			if ( 'add' == $_POST['event_attendance'] ) {
				...
				$p2p = self::modify_attendance( $event_id, $user_id, 'add' );
			} else {
				$notice = __("Thanks, please attend again soon! :)", APP_TD );
				$p2p = self::modify_attendance( $event_id, $user_id, 'remove' );
			}
		...
		$result = array(
			'link' 	 	=> self::create_attendance_link( $event_id, $user_id ),
			'banner' 	=> self::create_attendance_banner( $event_id, $user_id ),
			'attendees'	=> self::create_attendance_list( $event_id ),
			'status' 	=> $status,
			'notice' 	=> $notice,
			'redirect' 	=> $redirect,
		);
	...
	function create_attendance_link( $event_id, $user_id ) {
		...
		$attending = self::is_attending( $event_id, $user_id );
...
class VA_Widget_Upcoming_Events extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$days = array();
		for( $x = 1 ; $x <= 30 /* Could make this a widget option */; $x++ ) {
