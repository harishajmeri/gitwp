...
class VA_Event_Location_Meta extends scbPostMetabox {

	public function __construct(){
		parent::__construct( 'event-location', __( 'Location', APP_TD ), array(
...
class VA_Event_Contact_Meta extends scbPostMetabox {

	public function __construct(){
		parent::__construct( 'event-contact', __( 'Contact Information', APP_TD ), array(
			'post_type' => VA_EVENT_PTYPE,
			'context' => 'normal'
		) );
	}
...
class VA_Event_Dates_Meta extends APP_Meta_Box{
	...
	public function before_form( $post = '' ) {
		...
		if ( !empty( $days ) ) {
			foreach( $days as $date_U => $term ) {
				...
				$_blank_date = mysql2date( $date_format, $_date );
				$date = mysql2date( 'm/d/Y', $_date );
				?>
				...
				<tr class="event_day" id="event_day_<?php echo $day_count; ?>">
					<td class="date">
						...
						<label>
							<input type="text" class="regular-text" id="_blank_event_day_<?php echo $day_count; ?>" name="_event_day[<?php echo $day_count; ?>][_blank_date]" value="<?php echo esc_attr( $_blank_date ); ?>" />
						</label>
					</td>
					<td class="time-start">
						<label>
...
class VA_Event_Comments_Status_Meta extends scbPostMetabox {

	public function __construct(){
		parent::__construct( 'event-comments', __( 'Comments Status', APP_TD ), array(
			'post_type' => VA_EVENT_PTYPE
		) );
	}
...
class VA_Event_Publish_Moderation extends scbPostMetabox {

	public function __construct() {
		parent::__construct( 'event-publish-moderation', __( 'Moderation Queue', APP_TD ), array(
			'post_type' => VA_EVENT_PTYPE,
			'context' => 'side',
			'priority' => 'high'
		) );
...
class VA_Event_Author_Meta extends scbPostMetabox {

	public function __construct() {
		parent::__construct( 'eventauthordiv', __( 'Author', APP_TD ), array(
			'post_type' => VA_EVENT_PTYPE,
			'context' => 'side',
			'priority' => 'low'
		) );
...
class VA_Event_Featured_Meta extends scbPostMetabox {

	public function __construct(){
		parent::__construct( 'event-featured', __( 'Featured Information', APP_TD ), array(
			'post_type' => VA_EVENT_PTYPE,
			'context' => 'normal',
			'priority' => 'low'
		) );
