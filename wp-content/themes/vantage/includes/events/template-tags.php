...
function get_the_event_categories( $event_id = 0 ) {
	...
	$terms = get_the_terms( $event_id, VA_EVENT_CATEGORY );
...
function va_get_the_event_days_list( $event_id = 0, $date_format = '', $sep = ', ' ) {
	...
	foreach ( $event_days as $day )
		$days[] = date( $date_format, strtotime( $day->slug ) );
...
function va_get_the_event_days_span( $event_id = 0, $date_format = '', $sep = ' - ' ) {
	...
	if ( count( $event_days ) == 1 ) {
		$span = date( $date_format, strtotime( reset( $event_days )->slug ) );
		return $span;
	}

	$span = date( $date_format, strtotime( reset( $event_days )->slug ) );
	$span .= $sep;
	$span .= date( $date_format, strtotime( end( $event_days )->slug ) );
...
function va_get_the_event_day( $event_id = 0, $date_format = '') {
	...
	return date( $date_format, strtotime( reset( $event_days )->slug ) );
}
...
function va_get_the_event_cal_thumbnail( $event_id = 0 ) {
	...
	$day = html( 'div', array( 'class' => 'month' ), strtoupper( date( 'M' , strtotime( $days->slug ) ) . '.' ) );
			...
			<tr class="event_day" id="event_day_<?php echo $day_count; ?>">
				<td class="date">
					...
					<label>
						...
						<input type="text" class="<?php echo $required; ?>regular-text" id="_blank_event_day_<?php echo $day_count; ?>" name="_event_day[<?php echo esc_attr( $day_count ); ?>][_blank_date]" value="<?php echo $_blank_date; ?>" />
					</label>
				</td>
				<td class="time-start">
					<label>
