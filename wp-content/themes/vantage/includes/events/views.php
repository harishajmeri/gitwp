...
class VA_Event_Taxonomy_404 extends APP_View {
	...
	function breadcrumbs( $trail ) {
		...
		$new_trail['trail_end'] = va_format_event_day( va_get_search_query_var( VA_EVENT_DAY ) );
		return $new_trail;
	}
...
class VA_Event_Create extends APP_View_Page {
	...
	function __construct() {
		...
		add_action( 'wp_ajax_vantage_create_event_geocode', array( __CLASS__, 'handle_ajax' ) );
		add_action( 'wp_ajax_nopriv_vantage_create_event_geocode', array( __CLASS__, 'handle_ajax' ) );
	}
	...
	function template_include( $path ) {
		...
		if ( !is_user_logged_in() ) {
			...
			wp_redirect( add_query_arg( array( 'redirect_to' => urlencode(va_get_event_create_url()) ), APP_Login::get_url('redirect') ) );
			exit();
		}
