...
function va_format_event_day( $term ) {
	...
	if ( 7 == strlen( $term )  ) {
		$date_format = str_ireplace( array('/d','d/','j, ','j,','-d','d-'), '', get_option( 'date_format' ) );
	} else {
	...
	return apply_filters( 'va_event_day_single_term_title', date( $date_format, strtotime( $term ) ), $date_format, $term );
}
