...
add_action( 'init', '_va_pre_load_events', 4 );
add_action( 'init', '_va_load_events', 5 );
add_action( 'widgets_init', '_va_load_widgets', 1 );
add_action( 'init', '_va_load_events_admin_settings', 6 );
...
function _va_load_events() {
	...
	require dirname( __FILE__ ) . '/favorites.php';
	...
	require dirname( __FILE__ ) . '/views-checkout.php';
	new VA_Event_Gateway_Select;
	new VA_Event_Gateway_Process;
	new VA_Event_Order_Summary;


	require dirname( __FILE__ ) . '/custom-forms.php';

	require dirname( __FILE__ ) . '/event-form.php';
	new VA_Event_Info_Edit;
	new VA_Event_Info_Purchase;

	require dirname( __FILE__ ) . '/event-purchase.php';
	new VA_Select_Event_Plan_New;
	new VA_Select_Event_Plan_Existing;

	require dirname( __FILE__ ) . '/event-status.php';
	require dirname( __FILE__ ) . '/event-activate.php';

	if ( is_admin() ) {
		require dirname( __FILE__ ) . '/admin/admin.php';
		require_once dirname( __FILE__ ) . '/admin/settings.php';
		require dirname( __FILE__ ) . '/admin/pricing.php';
		require dirname( __FILE__ ) . '/admin/event-single.php';

		new VA_Event_Dates_Meta;
		new VA_Event_Location_Meta;
		new VA_Event_Contact_Meta;
		new VA_Event_Comments_Status_Meta;
		new VA_Event_Publish_Moderation;
		new VA_Event_Author_Meta;
		new VA_Event_Featured_Meta;
	}

	va_register_sidebar( 'single-event', __( 'Single Event Sidebar', APP_TD ), __( 'The sidebar for single Event page', APP_TD ) );

	new VA_Event_Create;
	new VA_Event_Purchase;
	new VA_Event_Edit;
	new VA_Event_Taxonomy;
	new VA_Event_Single;
	new VA_Event_Search;
	new VA_Event_Archive;
	new VA_Event_Categories;
	new VA_Event_Taxonomy_404;
}
