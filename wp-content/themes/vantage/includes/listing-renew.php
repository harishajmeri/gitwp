...
class VA_Renew_Listing extends VA_Checkout_Step {
	...
	function add_renew_to_order( $order ) {
		$order->add_item( VA_LISTING_RENEW_ITEM, 0, va_get_renewed_listing()->post->ID );
	}
...
class VA_Renew_Listing_Categories extends VA_Checkout_Step {
	...
	function add_category_selections( $order, $post_id ) {
		...
		add_post_meta( $order->get_id(), 'renew_custom_forms', $custom_forms, true );
	...
	function add_category_surcharges( $order, $post_id ) {
		...
		foreach ( $category_ids as $category_id ) {
			...
			if ( !empty( $surcharge ) ) {
				$order->add_item( VA_LISTING_CATEGORY . '_' . $category_id , $surcharge, $post_id );
			}
		}
	}
...
class VA_Renew_Listing_Select_Plan extends VA_Checkout_Step{
