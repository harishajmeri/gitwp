...
class VA_Listing_Info_Edit extends VA_Checkout_Step{
...
function va_add_non_plan_to_order( $order, $listing_id ){
	$order->add_item( VA_LISTING_PTYPE, 0, $listing_id );
}
...
function va_add_category_surcharges( $order, $post_id, $taxonomy ) {
	...
	foreach ( $selected_categories as $category ) {
		...
		if ( !empty( $surcharge ) ) {
			$order->add_item( $taxonomy . '_' . $category->term_id , $surcharge, $post_id );
		}
	}
}
